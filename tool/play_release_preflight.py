...
def require_exact_alarm_declaration(manifest: Path) -> None:
    permissions = manifest_permissions(manifest)
    uses_exact_alarm = "android.permission.USE_EXACT_ALARM" in permissions
    if not uses_exact_alarm:
        print("Exact alarm preflight: USE_EXACT_ALARM not declared.")
        return
...
def main() -> int:
    ...
    parser.add_argument("--package-name", default="com.almarfa.alquran")
    ...
    require_exact_alarm_declaration(args.manifest)
