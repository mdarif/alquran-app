...
ANDROID_PERMS = """    <!-- Sunnah reminders (local notifications) with EXACT-time delivery.
         USE_EXACT_ALARM (Android 13+) is granted at install and is the one that
         actually fires a salat nudge at the minute: SCHEDULE_EXACT_ALARM is NOT
         auto-granted on Android 14+, so with it alone the app silently degraded
         to inexact alarms with a delivery window of up to an HOUR (confirmed by
         `dumpsys alarm` on OxygenOS: window=+1h0m0s, flags=0x8) — late, batched
         and, in Doze, posted without the alert. Prayer times are wall-clock
         alarms, which is exactly the USE_EXACT_ALARM use case. Keep the older
         SCHEDULE_EXACT_ALARM capped at API 32 (Play rejects declaring both
         unbounded). REQUEST_IGNORE_BATTERY_OPTIMIZATIONS lets the app prompt to
         exempt itself so aggressive OEMs don't freeze the alarm.
         Re-apply after a `flutter create` regen — see `make notif-perms`. -->
